...
// Hardcoded state for the local server
let MOCK = {
    cohortInfo: {
...
app.get('/cohort/info', (req, res) => res.json(MOCK.cohortInfo));
app.get('/cohort/heatmap', (req, res) => res.json(MOCK.heatmap));
app.get('/cohort/at-risk', (req, res) => res.json(MOCK.atRisk));
app.get('/cohort/mastery', (req, res) => res.json(MOCK.mastery));
app.get('/cohort/curriculum', (req, res) => res.json(MOCK.curriculum));
app.get('/cohort/stats', (req, res) => res.json(MOCK.weeklyStats));
app.get('/cohort/homework', (req, res) => res.json(MOCK.homework));
...
app.post('/cohort/homework/:id/close', (req, res) => {
    const hw = MOCK.homework.find(h => h.id === req.params.id);
    if (hw) hw.status = 'closed';
    res.json({ ok: true });
});
...
app.listen(PORT, () => {
    console.log(`[Synapse Local Backend] Running on http://localhost:${PORT}`);
    console.log(`Available routes: /cohort/info, /cohort/heatmap, /cohort/at-risk, /cohort/mastery, /cohort/curriculum, /cohort/stats, /cohort/homework`);
});
